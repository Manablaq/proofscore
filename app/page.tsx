  ...
  return <main>
    ...
    {selected && <section className="section detail">
      ...
      {selected.status === 'OPEN' && selected.deadline * 1000 >= renderedAt && <form className="glass-form" onSubmit={submitProfile}>
        <div className="form-title"><span>Builder evidence</span><small>Source identifiers and context are assessed by validators.</small></div>
        ...
        <button className="button primary" disabled={!isConnected || !PROOFSCORE_IS_CONFIGURED}>Submit for validator scoring</button>
      </form>}
      ...
      <div className="submission-list">
        ...
        {submissions.length === 0 ? <div className="empty">No accepted submissions yet.</div> : submissions.map(submission => <article className="submission" key={submission.submission_id}>
          <div className="submission-main"><ScoreRing score={submission.score} /><div className="submission-copy"><div className="badges"><span className={`decision ${submission.decision.toLowerCase()}`}>{submission.decision.replace('_', ' ')}</span>{submission.eligible_to_claim && <span className="decision eligible">ELIGIBLE TO CLAIM</span>}{submission.claimed && <span className="decision claimed">CLAIMED · FINALIZATION DEPENDENT</span>}{submission.challenge_count > 0 && <span className="decision challenged">CHALLENGED</span>}{submission.revision_count > 0 && <span className="decision revised">REVISED</span>}</div><h3>{submission.handle}</h3><span className="wallet">Builder {short(submission.builder)} · Confidence {submission.confidence}</span><p>{submission.evidence_summary}</p><details><summary>Validator record</summary><p>{submission.reasoning}</p><div className="source-columns"><div><b>Accepted sources</b>{submission.accepted_sources.map(source => <span key={source}>{source}</span>)}</div><div><b>Risk flags</b>{submission.risk_flags.length ? submission.risk_flags.map(flag => <span key={flag}>{flag}</span>) : <span>None recorded</span>}</div></div></details></div></div>
          <div className="dimensions">{Object.entries(submission.dimensions).map(([name, value]) => <div key={name}><span>{name}</span><i><b style={{ width: `${value * 5}%` }} /></i><strong>{value}/20</strong></div>)}</div>
    ...
    <section className="section create" id="create"><div><span className="eyebrow">SPONSOR A SETTLEMENT</span><h2>Create and fund a campaign</h2><p>The deposited GEN becomes the campaign reward pool. Every accepted score at or above your threshold unlocks one fixed claim, while counter-evidence can revise eligibility before claim.</p></div><form className="glass-form" onSubmit={createCampaign}><div className="form-grid"><label>Campaign title<input required name="title" minLength={3} maxLength={100} /></label><label>Minimum ProofScore<input required name="threshold" type="number" min="1" max="100" defaultValue="70" /></label><label>Reward per builder (GEN)<input required name="reward" type="number" min="0.000001" step="0.000001" /></label><label>Funded reward slots<input required name="slots" type="number" min="1" defaultValue="3" /></label><label>Deadline<input required name="deadline" type="datetime-local" /></label><label className="wide">Description<textarea required name="description" /></label><label className="wide">Evidence requirements<textarea required name="requirements" placeholder="Required source types, signals, and minimum proof quality." /></label></div><button className="button primary" disabled={!isConnected || !PROOFSCORE_IS_CONFIGURED}>Create campaign + deposit pool</button><small>Payable value is sent in wei. The transaction hash is immediate; campaign state appears only after acceptance.</small></form></section>
